...
*
* wp-admin/options-general.php?page=adhc-mybama-login&.. 
*/
...
<div class="wsl-container">
		...
		if( get_option( 'wsl_settings_development_mode_enabled' ) )
		{
			?>
				<div class="fade error wsl-error-dev-mode-on" style="margin: 4px 0 20px;">
					...
					<p>
						<a class="button-secondary" href="options-general.php?page=adhc-mybama-login&wslp=tools#dev-mode"><?php _wsl_e('Change this mode', 'wordpress-social-login') ?></a>
		...
		if( get_option( 'wsl_settings_debug_mode_enabled' ) )
		{
			?>
				<div class="fade updated wsl-error-debug-mode-on" style="margin: 4px 0 20px;">
					...
					<p>
						<a class="button-secondary" href="options-general.php?page=adhc-mybama-login&wslp=tools#debug-mode"><?php _wsl_e('Change this mode', 'wordpress-social-login') ?></a>
						<a class="button-secondary" href="options-general.php?page=adhc-mybama-login&wslp=watchdog"><?php _wsl_e('View WSL logs', 'wordpress-social-login') ?></a>
	...
	<h1 <?php if( is_rtl() ) echo 'style="margin: 20px 0;"'; ?>>
		<?php _wsl_e( 'ADHC MyBama Login', 'adhc-mybama-login' ) ?>
		...
		<?php
			...
			foreach( $WORDPRESS_SOCIAL_LOGIN_ADMIN_TABS as $name => $settings )
			{
				if( $settings["enabled"] && ( $settings["visible"] || $wslp == $name ) )
				{
					...
					?><a class="nav-tab <?php if( $wslp == $name ) echo "nav-tab-active"; ?>" style="<?php echo $css_pull_right; ?>" href="options-general.php?page=adhc-mybama-login&wslp=<?php echo $name ?>"><?php if( isset( $settings["ico"] ) ) echo '<img style="margin: 0px; padding: 0px; border: 0px none;width: 16px; height: 16px;" src="' . WORDPRESS_SOCIAL_LOGIN_PLUGIN_URL . '/assets/img/' . $settings["ico"] . '" />'; else _wsl_e( $settings["label"], 'adhc-mybama-login' ); ?></a><?php
				}
			}
		?>
	</h2>
...
<div id="wsl_div_warn">
	...
	<div>
		...
		<a class="button-primary" href="options-general.php?page=adhc-mybama-login&wslp=components" style="float:<?php if( is_rtl() ) echo 'left'; else echo 'right'; ?>"><?php _wsl_e( "Check enabled components", 'adhc-mybama-login' ) ?></a>
	</div> 
</div>  
<?php
	// HOOKABLE: 
...
{
	add_options_page('ADHC MyBama Login', 'ADHC MyBama Login', 'manage_options', 'adhc-mybama-login', 'wsl_admin_main' );
